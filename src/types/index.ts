...
export type AdProviderType = 'adsterra' | 'monetag' | 'telegram' | 'custom' | 'google_admanager';

export type RewardedAdProviderType = 'monetag' | 'custom' | 'telegram' | 'mock';

export interface RewardedMonetagConfig {
  zoneId: string;
  sdkFunctionName: string;
  scriptUrl?: string;
}

export interface RewardedCustomConfig {
  html: string;
  completionType?: 'message_event' | 'button_callback';
  eventKey?: string;
}

export interface RewardedAdConfig {
  provider: RewardedAdProviderType;
  enabled: boolean;
  monetag: RewardedMonetagConfig;
  custom: RewardedCustomConfig;
