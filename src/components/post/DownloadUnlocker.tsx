...
const handleDownloadClick = () => {
  if (!isUnlocked || downloadCooldown > 0) return;
  ...
  try {
    ...
    if (isTelegramLink) {
      ...
      if (telegramWebApp) {
        /*
         * IMPORTANT:
         * We pass the COMPLETE URL here.
         *
         * Example:
         * https://t.me/abcbot?start=abc
         *
         * The ?start=abc is NOT removed or modified.
         */
        telegramWebApp.openTelegramLink(url);

        /*
         * Do NOT call WebApp.close() immediately.
         *
         * Telegram handles the navigation away from the Mini App.
         */
        return;
      }
    ...
    // Normal web URL → same behavior as before
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-4 border-b border-slate-200 dark:border-slate-800">
        <div>
          ...
          <h3 className="text-lg sm:text-xl font-bold text-slate-900 dark:text-slate-100">
            {isUnlocked
              ? "Download Unlocked & Verified"
              : "Complete Verification Steps to Unlock"}
          </h3>
        </div>
        ...
        <div>
          {totalRequired === 0 ? (
            ...
            <span className="px-3 py-1 rounded-full text-xs font-bold bg-emerald-500/10 dark:bg-emerald-500/20 text-emerald-700 dark:text-emerald-300 border border-emerald-500/30 flex items-center gap-1.5 animate-pulse">
              <CheckCircle2 className="w-3.5 h-3.5" />
              <span>
                All Steps Completed ({completedSteps}/{totalRequired})
              </span>
            </span>
          ) : (
            <span className="px-3 py-1 rounded-full text-xs font-bold bg-amber-500/10 dark:bg-amber-500/20 text-amber-700 dark:text-amber-300 border border-amber-500/30 flex items-center gap-1.5">
              ...
              <span>
                Locked ({completedSteps}/{totalRequired} Steps Done)
              </span>
            </span>
          )}
        </div>
        ...
        <div className="space-y-2">
          <div className="flex items-center justify-between text-xs text-slate-500 dark:text-slate-400">
            ...
            <span className="font-semibold text-slate-900 dark:text-slate-200">
              {Math.round((completedSteps / totalRequired) * 100)}% (
      ...
      {totalRequired > 0 && !isUnlocked && (
        <div className="space-y-3">
          <p className="text-xs text-slate-500 dark:text-slate-400">
            Watch the sponsored advertisement step{totalRequired > 1 ? "s" : ""}{" "}
            below to generate your secure download link:
          </p>
        ...
        {isUnlocked ? (
          <div className="space-y-3">
            <button
              ...
              className={`w-full py-4 px-6 rounded-2xl text-white font-extrabold text-base transition-all shadow-xl flex items-center justify-center gap-3 transform active:scale-[0.99] ${
                downloadCooldown > 0
                  ? "bg-slate-500 shadow-slate-500/20 cursor-not-allowed opacity-80"
                  : "bg-gradient-to-r from-emerald-600 via-emerald-500 to-teal-500 hover:from-emerald-500 hover:to-teal-400 shadow-emerald-600/30 cursor-pointer"
              }`}
            >
              {downloadCooldown > 0 ? (
                <>
                  <span className="w-6 h-6 rounded-full border-2 border-white/30 border-t-white animate-spin" />
                  <span>Please wait {downloadCooldown}s</span>
                </>
              ) : (
                <>
            ...
            <p className="text-[11px] text-center text-slate-500">
              Direct secure link ready. Verified virus-free and checksum
              inspected.
            </p>
          </div>
        ) : (
          ...
          >
            ...
            <span>
              Download Locked — Complete {totalRequired - completedSteps} More
              Step(s)
            </span>
          </button>
        )}
