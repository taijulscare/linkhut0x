      ...
      <div className="relative w-full max-w-2xl bg-slate-900 border border-slate-800 rounded-2xl shadow-2xl overflow-hidden z-10">
        ...
        <div className="flex items-center px-4 py-3.5 border-b border-slate-800 bg-slate-950/60">
          ...
          <button
            ...
            type="button"
            onClick={onClose}
            className="ml-2 px-2 py-1 text-xs font-semibold rounded-lg bg-slate-800 text-slate-400 hover:text-white"
          >
            ESC
          </button>
        </div>
        ...
        <div className="max-h-[60vh] overflow-y-auto p-3 divide-y divide-slate-800/60">
          ...
          {!loading && searchTerm && results.length === 0 && (
            <div className="py-10 text-center text-xs text-slate-400">
              No results found for "<span className="text-slate-200">{searchTerm}</span>". Try another keyword.
            </div>
          )}
          ...
          {!loading && results.map((post) => (
            <div
              ...
              className="p-3 rounded-xl hover:bg-slate-800/70 transition-all cursor-pointer flex items-center gap-3 group"
            >
              <img
              ...
              <div className="flex-1 min-w-0">
                ...
                <h4 className="font-semibold text-xs sm:text-sm text-slate-100 truncate group-hover:text-indigo-400 transition-colors">
                  {post.title}
                </h4>
