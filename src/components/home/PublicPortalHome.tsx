      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-8">
        ...
        {selectedCategorySlug === 'all' && featuredPost && (
          <div
            ...
            className="group relative rounded-3xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 hover:border-indigo-500/60 overflow-hidden cursor-pointer shadow-lg hover:shadow-2xl transition-all"
          >
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-0">
              {/* Image Preview */}
              <div className="lg:col-span-7 relative aspect-video lg:aspect-auto overflow-hidden bg-slate-100 dark:bg-slate-950 min-h-[260px] sm:min-h-[340px]">
                <img
                ...
                />
                <div className="absolute inset-0 bg-gradient-to-t lg:bg-gradient-to-r from-slate-950/80 lg:from-slate-950/90 via-slate-950/30 to-transparent" />
              ...
              {/* Text Info */}
              <div className="lg:col-span-5 p-6 sm:p-8 flex flex-col justify-between space-y-4 bg-gradient-to-br from-slate-50 to-white dark:from-slate-900 dark:to-slate-950">
                <div className="space-y-3">
                  <div className="flex items-center gap-2">
                    <span className="text-[11px] font-bold px-2.5 py-0.5 rounded-lg bg-indigo-500/10 text-indigo-700 dark:text-indigo-300 border border-indigo-500/20">
                  ...
                  <h2 className="text-xl sm:text-2xl font-extrabold text-slate-900 dark:text-slate-100 group-hover:text-indigo-600 dark:group-hover:text-indigo-400 transition-colors leading-snug">
                    {featuredPost.title}
                  </h2>
                ...
                <div className="pt-4 border-t border-slate-200 dark:border-slate-800/80 flex items-center justify-between">
                  <div className="flex items-center gap-2 text-xs text-slate-500 dark:text-slate-400">
                    <span className="px-2 py-0.5 rounded bg-slate-100 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 text-amber-700 dark:text-amber-300 font-bold text-[11px]">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pt-2">
            ...
            <h3 className="text-lg sm:text-xl font-bold text-slate-900 dark:text-slate-100 flex items-center gap-2">
              ...
              <span>
                {selectedCategorySlug === 'all'
                  ? 'All Latest Videos & Story'
                  : `${categories.find(c => c.slug === selectedCategorySlug)?.name || 'Category'} Catalog`}
              </span>
            </h3>
            <p className="text-xs text-slate-500 dark:text-slate-400">
              Verified premium video collections with fast download acceleration and step-based reward unlocks.
            </p>
          </div>
          ...
          <button
            ...
            className="py-2 px-4 rounded-xl bg-white dark:bg-slate-900 hover:bg-slate-50 dark:hover:bg-slate-850 border border-slate-200 dark:border-slate-800 text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-slate-200 text-xs font-semibold flex items-center justify-between gap-4 transition-all shadow-sm group"
          >
            <div className="flex items-center gap-2">
            ...
            <kbd className="px-1.5 py-0.5 rounded bg-slate-100 dark:bg-slate-950 text-[10px] text-slate-500 font-mono border border-slate-200 dark:border-slate-800">
              ⌘K
            </kbd>
          </button>
        </div>
            ...
            {hasMore && (
              <div className="flex justify-center pt-6">
                <button
                  ...
                  className="py-3 px-8 rounded-2xl bg-white dark:bg-slate-900 hover:bg-slate-50 dark:hover:bg-slate-850 border border-slate-200 dark:border-slate-800 text-slate-700 dark:text-slate-200 hover:text-slate-900 dark:hover:text-white text-xs font-bold transition-all shadow-md hover:shadow-indigo-500/10 flex items-center gap-2 cursor-pointer"
                >
                  {loadingMore ? (
                    <>
