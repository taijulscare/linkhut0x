...
/**
 * Fetch a single post by slug or ID (cached for public visits)
 */
export async function getPostBySlug(slugOrId: string): Promise<Post | null> {
  if (!slugOrId) return null;
  const cacheKey = `post_slug_${slugOrId}`;
  const cached = appCache.get<Post>(cacheKey);
  if (cached) return cached;

  try {
    // 1. Primary public query: match slug and published == true
    // This strictly aligns with Firestore security rules for unauthenticated/public reads
    const q = query(
      collection(db, POSTS_COLLECTION),
      where('slug', '==', slugOrId),
      where('published', '==', true),
    ...
    // 2. Direct document ID lookup fallback (for /post/POST_ID URLs)
    try {
      const directRef = doc(db, POSTS_COLLECTION, slugOrId);
      const directSnap = await getDoc(directRef);
      if (directSnap.exists()) {
        const data = directSnap.data();
        if (data && (data.published === true || auth.currentUser)) {
          const post = { id: directSnap.id, ...data } as Post;
          appCache.set(cacheKey, post, CACHE_TTL);
          return post;
        }
      }
    } catch (docErr) {
      // Direct doc lookup ignored if not found / not a valid doc id
    }

    // 3. Fallback for Admin users viewing drafts by slug
    if (auth.currentUser) {
      try {
        const adminQ = query(
          collection(db, POSTS_COLLECTION),
        ...
        // Ignore admin draft query error
      }
    }

    return null;
  } catch (err) {
    console.error(`Error fetching post by slug (${slugOrId}):`, err);
    return null;
  }
}
...
export async function seedInitialSampleData(): Promise<{ categoriesCount: number; postsCount: number; adsCount: number }> {
  ...
  for (const cat of defaultCategories) {
    ...
    if (snap.empty) {
      ...
      await setDoc(docRef, cleanFirestoreData(catObj));
...
Ready-to-use props, glowing neon signs, cybernetic vehicles, and architectural modules.`,
  ...
  for (const post of samplePosts) {
    ...
    if (snap.empty) {
      ...
      await setDoc(postRef, cleanFirestoreData({
        id: postRef.id,
        title: post.title,
        slug: post.slug,
  ...
  for (const ad of defaultAds) {
    ...
    if (snap.empty) {
      ...
      await setDoc(adRef, cleanFirestoreData({
        id: adRef.id,
        ...ad,
        createdAt: Date.now(),
        updatedAt: Date.now()
