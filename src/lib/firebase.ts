...
/**
 * Recursively cleans any object to guarantee no `undefined` values reach Firestore.
 * Preserves special Firestore FieldValues, Timestamps, and arrays.
 */
export function cleanFirestoreData<T>(data: T): T {
  ...
  if (typeof data === 'object') {
    // Preserve Firestore serverTimestamp, FieldValue, Timestamp classes
...
export async function syncUserProfile(user: FirebaseUser): Promise<UserProfile> {
  ...
  if (!snapshot.exists()) {
    ...
    if (isPrimaryAdmin && data.role !== 'admin') {
      await setDoc(userRef, cleanFirestoreData({ role: 'admin', updatedAt: Date.now() }), { merge: true });
    }
